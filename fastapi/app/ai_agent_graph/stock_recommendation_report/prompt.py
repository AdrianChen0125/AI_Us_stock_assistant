...
def build_prompt(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        "4. **Buy Opportunity Check**: From the table above, assess if any recommended stock is close to its 52-week low (within 15%). Highlight these as potential entry points and explain why now may be a good time to invest (or not).\n\n"
        "5. **Alternatives & Similar Stocks**: Suggest other stocks that are either similar in profile or from the same sector, especially if they offer a better risk fit.\n\n"
        "6. **Diversification Tips**: Recommend 2–3 additional S&P 500 stocks from different sectors to improve portfolio diversification.\n\n"
        "7. **Portfolio Allocation Strategy**: Based on the investor’s risk tolerance, propose a portfolio breakdown (e.g., % in growth vs. defensive), and highlight any key risks.\n\n"
